...
public class PathFinder
{
    ...
    {
        if (animObj.curCam == null || !GameState.Project.Cameras.TryGetValue(animObj.curCam, out var value)) return;
        ...
        if (curPath.ID != null && curPath.Type == "camera" &&
            GameState.Project.Cameras.TryGetValue(curPath.ID, out var prevCamera))
            prevCamera.CurState = string.Join(",",
                prevCamera.CurState.Split(',').Where(s => s != anim).DefaultIfEmpty("Default"));
    }
