    ...
    public static class MenuUtils
    {
        ...
        public static class Element
        {
            ...
            public static bool IsElementSelected(string elementID)
            {
                ...
                {
                    { } => GameCache.Buttons.TryGetValue(elementID, out var btn) && btn.IsHovered,
                    _ => false
                };
            }

            public static bool HideElement(string elementID) => SetElementVisibility(elementID, true);

            public static bool ShowElement(string elementID) => SetElementVisibility(elementID, false);

            public static bool SetSprite(string elementID, string newSprite) => SetElementSprite(elementID, newSprite);
            ...
            public static void DrawMenuElements()
            {
                ...
                {
                    if (el.Hidden) return;
                    string uid = $"{el.Text}-{el.FontSize}";

                    Vector2 ElPos = new(el.X, el.Y);
                    switch (el.Type)
                    {
                        case "StaticText":
                            if (GameCache.Texts.TryGetValue(uid, out var text)) text.Draw(ElPos);
                            else
                            {
                            ...
                            break;
                        case "Button":
                            if (GameCache.Buttons.TryGetValue(el.ID, out var btn)) btn.Draw(ElPos);
                            else
                            {
                                Text tx = GameCache.Texts.TryGetValue(uid, out Text? value) ? value : GameCache.Texts[uid] = new Text(el.Text, el.FontSize, el.FontName, Raylib.WHITE);
                            ...
                            break;
                        case "Image":
                            if (GameCache.Buttons.TryGetValue(el.ID, out var imgbtn)) imgbtn.Draw(ElPos);
                            else
                            {
