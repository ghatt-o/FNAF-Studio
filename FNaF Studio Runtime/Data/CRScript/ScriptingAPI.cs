...
namespace FNAFStudio_Runtime_RCS.Data.CRScript
{
    public class ScriptingAPI
    {
            ...
            Actions = new Dictionary<string, Func<List<string>, bool>>
        {
            ...
            // Object manipulation
            { "hide_office_object", HideOfficeObject }, // Hides door buttons, idk why - HexDev // Keep it that way. - Mike
        ...
        bool IfCameraUp(List<string> args)
        {
        ...
        bool IfCurrentCamera(List<string> args)
        {
            if (OfficeCore.OfficeState != null)
        ...
        static bool ChangeCamera(List<string> args)
        {
            if (OfficeCore.OfficeState != null && OfficeCore.OfficeState.Cameras.ContainsKey(args[0]))
            {
                ...
                Logger.LogFatalAsync("ScriptingAPI", "OfficeState is null or invalid argument provided for Change Camera. (" + args[0] + ")");
            return false;
        }

        bool SetTime(List<string> args)
        {
            if (args.Count < 1)
            {
            ...
            int hours = RuntimeUtils.ParseInt(args[0]);
            if (hours > -1 && hours < 7)
            {
            ...
            else if (hours == 12)
            {
                ...
                Logger.LogFatalAsync("ScriptingAPI", "Invalid argument provided for Set Time. Value must be a number above -1 and below 7.");
            return false;
        }

        bool SetPowerUsage(List<string> args)
        {
            if (args.Count < 1)
            {
            ...
            int usage = RuntimeUtils.ParseInt(args[0]);
            if (OfficeCore.OfficeState != null)
            {
        ...
        bool SetPowerLevel(List<string> args)
        {
            if (args.Count < 1)
            {
            ...
            int level = RuntimeUtils.ParseInt(args[0]);
            if (OfficeCore.OfficeState != null)
            {
        ...
        bool SendMsg(List<string> args)
        {
            string? msg = args.FirstOrDefault();
            if (msg != null)
        ...
        {
            int nightValue = newGame ? 1 : EventManager.dataValues.TryGetValue("Night", out string? value) ? int.Parse(value ?? "1") : night;
            if (newGame) EventManager.SetDataValue("Night", "1");
            OfficeUtils.StartOffice(nightValue);
            return true;
        }

        bool StartNewGame(List<string> args)
        {
            return StartNight(true, -1);
        }

        bool ContinueGame(List<string> args)
        {
            return StartNight(false, -1);
        }

        bool StartNightGame(List<string> args)
        {
            return StartNight(false, RuntimeUtils.ParseInt(EventManager.GetExpr(args[0])));
        }

        bool SetOff(List<string> args)
        {
            OfficeCore.Office = args[0];
            return true;
        }

        bool SetOffice(List<string> args)
        {
            if (OfficeCore.OfficeState != null)
            {
                OfficeCore.Office = args[0];
                OfficeCore.LoadingLock = true;
                int Night = OfficeCore.OfficeState.Night;
        ...
        bool IfOfficeState(List<string> args)
        {
            if (OfficeCore.OfficeState != null)
        ...
        bool SetOfficeState(List<string> args)
        {
            if (OfficeCore.OfficeState != null)
            {
                string arg = EventManager.GetExpr(args[0]);
        ...
        bool HideOfficeObject(List<string> args)
        {
        ...
        bool ShowOfficeObject(List<string> args)
        {
            if (OfficeCore.OfficeState != null && !OfficeCore.LoadingLock)
            {
                if (OfficeCore.OfficeState.Office.Objects.TryGetValue(EventManager.GetExpr(args[0]), out var Object))
        ...
        bool IsMouseOverObject(List<string> args)
        {
        ...
        bool IsMouseOverSprite(List<string> args)
        {
            if (GameCache.Buttons.TryGetValue(EventManager.GetExpr(args[0]), out var Button))
            {
                return Button.IsHovered;
        ...
        public static bool Quit(List<string> args) => RuntimeUtils.Quit();
        ...
        public static bool GotoMenu(List<string> args)
            => MenuUtils.GotoMenu(EventManager.GetExpr(args[0]));

        public static bool SetBackground(List<string> args)
            => MenuUtils.SetBackground(EventManager.GetExpr(args[0]));

        public static bool IsElementSelected(List<string> args)
            => MenuUtils.Element.IsElementSelected(EventManager.GetExpr(args[0]));

        public static bool HideElement(List<string> args)
            => MenuUtils.Element.HideElement(EventManager.GetExpr(args[0]));

        public static bool ShowElement(List<string> args)
            => MenuUtils.Element.ShowElement(EventManager.GetExpr(args[0]));

        public static bool SetText(List<string> args)
            => MenuUtils.Element.SetText(EventManager.GetExpr(args[0]), EventManager.GetExpr(args[1]));

        public static bool SetSprite(List<string> args)
            => MenuUtils.Element.SetSprite(EventManager.GetExpr(args[0]), EventManager.GetExpr(args[1]));
        ...
        {
            string lhs = EventManager.GetExpr(args[0].ToString());
            string rhs = EventManager.GetExpr(args[2].ToString());
        ...
        {
            var mouseButtons = new (MouseButton, string)[]
            {
