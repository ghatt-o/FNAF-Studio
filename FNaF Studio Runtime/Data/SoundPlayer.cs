...
namespace FNAFStudio_Runtime_RCS.Data
{
    public class SoundPlayer
    {
        ...
        {
            string soundsDir = Path.Combine(assetsPath, "sounds");
            if (Directory.Exists(soundsDir))
            {
                foreach (string file in Directory.GetFiles(soundsDir))
                {
        ...
        {
            if (Cache.Sounds.ContainsKey(id))
            {
                loopingSounds[id] = loop;
            }
        }
