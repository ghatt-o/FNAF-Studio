...
namespace Editor.Views
{
    public class SoundsView : IContent
    {
        ...
        public void Initialize()
        {
            ...
            GameJson.Sounds sounds = ProjectManager.Project.Sounds;
        ...
        public void Render()
        {
            ...
            ImGui.Spacing();
            ImGui.Spacing();
            RenderIndexedDropdown("Animatronic Movement", "Move Sound", ProjectManager.Project.Sounds.AnimatronicMove, ref selectedMoveSoundIndex);
            RenderIndexedDropdown("Phone Calls", "Night", ProjectManager.Project.Sounds.PhoneCalls, ref selectedPhoneCallIndex);
        ...
        private void RenderSoundTypeDropdown()
        {
            ...
            if (ImGui.BeginCombo("##soundTypeCombo", selectedSoundType))
            {
                if (availableSoundTypes != null)
                {
                    ...
                    {
                        bool isSelected = (soundType == selectedSoundType);
        ...
        private void HandleSelectedSound()
        {
            ...
            {
                string soundFile = soundFileMap[selectedSoundType];
                ImGui.SameLine();
                ...
                ImGui.SameLine();
                if (ImGui.Button($"Clear File##{selectedSoundType}"))
                {
                    soundFileMap[selectedSoundType] = string.Empty;
                }
            }
        }
        ...
        private void RenderIndexedDropdown(string label, string prefix, List<string> list, ref string selectedIndex)
        {
            ...
            string currentSelection = selectedIndex;

            if (ImGui.BeginCombo($"##{label}_Dropdown", currentSelection))
            {
                for (int i = 0; i < list.Count; i++)
                {
                    string indexLabel = $"{prefix} {i + 1}";
                    bool isSelected = (indexLabel == currentSelection);
            ...
            {
                int index = int.Parse(string.Concat(selectedIndex.Where(Char.IsDigit))) - 1;
                if (index >= 0 && index < list.Count)
                {
                    ...
                    if (ImGui.Button($"Clear File##{prefix}_{index}"))
                    {
        ...
        private void HandleFilePickerPopup()
        {
            if (showFilePickerPopup)
            {
                ...
                if (ImGui.BeginPopup("File Picker"))
                {
                    ...
                    {
                        string selectedFilePath = "TotallyARealFile.wav";
        ...
        private void AssignSelectedFilePath(string filePath)
        {
            ...
            {
                int index = int.Parse(string.Concat(currentField.Where(Char.IsDigit))) - 1;
                if (currentField.StartsWith("Move Sound"))
